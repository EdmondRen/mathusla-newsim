...
std::vector<std::unique_ptr<Tracker::DigiHit>> genHits(double x0_span, double y0_span, double z0, double Ax_span, double Ay_span, double v = 30, int n_layers = 6)
{
    ...
    std::vector<std::unique_ptr<Tracker::DigiHit>> hits;
    for (int i = 0; i < n_layers; i++)
    {
        auto dz = layer_zs[i] - layer_zs[0];
        double hit_truth[6] = {x0 + Ax * dz, y0 + Ay * dz, t0 + At * dz, Ax, Ay, At};
        if (i % 2 == 0)
            hits.push_back(std::make_unique<Tracker::DigiHit>(round(hit_truth[0] / det_width) * det_width,
            ...
            hits.push_back(std::make_unique<Tracker::DigiHit>(hit_truth[0] + rng.Gaus(0, unc_longi),
