...
merged_file_name2=$MERGE_DIR/series_${SERIES_NUMBER}_digi_recon_all.root
for irun in $(seq 1 $N_RUNS_PER_SERIES); do
    ...
    bash $SIM_REPO_DIR/studies/mathusla40/cosmic_mu/_start_single_run.sh $SIM_REPO_DIR $SIM_TEMP_DIR $N_EVENTS_PER_RUN $actual_run_number
    ...
    current_file_name_recon=$SIM_TEMP_DIR/run_${actual_run_number}_digi_recon_upward.root
    current_file_name_recon_all_vertices=$SIM_TEMP_DIR/run_${actual_run_number}_digi_recon_all.root

    # Merge the reconstruction result
    if ((irun > 1)); then
        ./merge $current_file_name_recon $merged_file_name "data"
        ./merge $current_file_name_recon_all_vertices $merged_file_name2 "data"
        ...
        cp  $current_file_name_recon_all_vertices $merged_file_name2
    fi
    ...
    if [[ ! -z "$CLEAN_FILES" && "$CLEAN_FILES" == "y" ]]; then
        ...
        rm -f $current_file_name_recon_all_vertices
